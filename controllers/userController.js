	...
	try {
		...
		if (!couple || couple.length < 2) {
			const error = new Error(
				"커플을 찾을 수 없거나 커플의 구성원이 2명 미만입니다."
			);
			error.status = 404;
			throw error;
		}
		...
		await User.findByIdAndUpdate(couple[0]._id, {
			$set: { connectCode: null },
		});

		// 커플의 두 번째 구성원(user2)의 `connectCode`를 `null`로 업데이트합니다.
		await User.findByIdAndUpdate(couple[1]._id, {
			$set: { connectCode: null },
		});
